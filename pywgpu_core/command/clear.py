...
def clear_buffer(
    ...
    if offset % wgt.COPY_BUFFER_ALIGNMENT != 0:
        raise ClearError(
            f"Buffer offset {offset} is not a multiple of COPY_BUFFER_ALIGNMENT"
        )

    size = size if size is not None else (dst_buffer.size - offset)
    if size % wgt.COPY_BUFFER_ALIGNMENT != 0:
        raise ClearError(
            f"Buffer clear size {size} is not a multiple of COPY_BUFFER_ALIGNMENT"
        )

    if offset + size > dst_buffer.size:
        raise ClearError(
            f"Clear of {offset}..{offset+size} would overrun the bounds of the buffer of size {dst_buffer.size}"
        )
    ...
    dst_pending = state.tracker.buffers.set_single(
        dst_buffer, wgt.BufferUsages.COPY_DST
    )
    ...
    state.raw_encoder.clear_buffer(dst_buffer.raw(), offset, size)
...
def clear_texture_cmd(
    ...
    full_layer_count = (
        dst_texture.desc.array_layer_count()
    )  # Assuming this exist or use getattr
        ...
        for array_layer in range_.layer_range:
            for z in range(depth):
                ...
                while num_rows_left > 0:
                    ...
                    zero_buffer_copy_regions.append(
        ...
        for array_layer in range_.layer_range:
            ...
            view_desc = hal.TextureViewDescriptor(
                ...
                usage=(
                    ...
                    if is_color
                    else TextureUses.DEPTH_STENCIL_WRITE
                ),
                range=hal.ImageSubresourceRange(
                    aspect=(
                        hal.FormatAspects.COLOR
                        if is_color
                        else hal.FormatAspects.DEPTH_STENCIL
                    ),
            ...
            view = dst_texture.device.hal_device.create_texture_view(
                dst_texture.raw(), view_desc
            )
            ...
            if is_color:
                color_attachments.append(
                    hal.ColorAttachment(
                        target=hal.Attachment(
                            view=view, usage=TextureUses.COLOR_TARGET
                        ),
                        depth_slice=None,
                        resolve_target=None,
                        ops=hal.AttachmentOps.STORE | hal.AttachmentOps.LOAD_CLEAR,
                        clear_value=hal.Color(
                            0.0, 0.0, 0.0, 0.0
                ...
                depth_stencil_attachment = hal.DepthStencilAttachment(
                    target=hal.Attachment(
                        view=view, usage=TextureUses.DEPTH_STENCIL_WRITE
                    ),
