...
class Queue:
    ...
    def submit(self, command_buffers: Any) -> None:
        ...
        if hal_queue and hasattr(hal_queue, "submit"):
            ...
            try:
                hal_queue.submit(
                    command_buffers, [], (None, 0)
                )  # Empty surface textures, no fence
    ...
    def write_buffer(self, buffer: Any, buffer_offset: int, data: Any) -> None:
        ...
        try:
            ...
            mapping = hal_device.map_buffer(
                buffer, range(buffer_offset, buffer_offset + len(data))
            )
            ...
            hal_device.flush_mapped_ranges(
                buffer, [range(buffer_offset, buffer_offset + len(data))]
            )
