...
def has_copy_partial_init_tracker_coverage(
    copy_size: dict, mip_level: int, desc: dict
) -> bool:
    """
    ...
    target_depth = (
        max(1, depth >> mip_level) if desc.get("dimension") == "3d" else depth
    )
    ...
    is_partial = (
        ...
        or copy_height != target_height
        or (desc.get("dimension") == "3d" and copy_depth != target_depth)
    )
...
class TextureInitTracker:
    ...
    def check_action(
        self, action: TextureInitTrackerAction
    ) -> Optional[TextureInitTrackerAction]:
        """
        ...
        for i in range(action.range.mip_range.start, action.range.mip_range.end):
            ...
            if uninitialized_layer_range is not None:
                ...
                layer_range_start = min(
                    layer_range_start, uninitialized_layer_range.start
                )
                layer_range_end = max(layer_range_end, uninitialized_layer_range.end)

        # Check if we found any uninitialized ranges
        if mip_range_start < mip_range_end and layer_range_start < layer_range_end:
