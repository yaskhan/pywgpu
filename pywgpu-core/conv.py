...
def map_buffer_usage(usage: Any) -> Any:
    ...
    def has_flag(flag_name: str) -> bool:
        if hasattr(wgt, 'BufferUsages') and hasattr(usage, '__contains__'):
            ...
            if flag is not None:
                return flag in usage or (hasattr(usage, 'value') and (usage.value & flag) != 0)
        return False
...
def map_texture_usage(
    ...
    def has_feature_flag(flag_name: str) -> bool:
        if hasattr(wgt, 'TextureFormatFeatureFlags') and hasattr(flags, '__contains__'):
            ...
            if flag is not None:
                return flag in flags or (hasattr(flags, 'value') and (flags.value & flag) != 0)
        return False
...
def map_texture_usage_for_texture(
    ...
    if hasattr(desc.format, 'is_depth_stencil_format') and desc.format.is_depth_stencil_format():
        ...
        if copy_dst and copy_dst in desc.usage:
            ...
            if hasattr(format_features, 'allowed_usages'):
                render_attachment = getattr(wgt.TextureUsages, 'RENDER_ATTACHMENT', None)
                if render_attachment and render_attachment in format_features.allowed_usages:
                    if hasattr(desc, 'dimension'):
                        # Render targets must be 2D
                        d2 = getattr(wgt.TextureDimension, 'D2', None) if hasattr(wgt, 'TextureDimension') else None
...
def map_texture_usage_from_hal(uses: Any) -> Any:
    ...
    def has_use(use_name: str) -> bool:
        if hasattr(wgt, 'TextureUses') and hasattr(uses, '__contains__'):
            ...
            if use_flag is not None:
                return use_flag in uses or (hasattr(uses, 'value') and (uses.value & use_flag) != 0)
        return False
    ...
    storage_uses = has_use('STORAGE_READ_ONLY') or has_use('STORAGE_WRITE_ONLY') or has_use('STORAGE_READ_WRITE')
...
def check_texture_dimension_size(
    ...
    if hasattr(wgt, 'TextureDimension'):
        ...
        if dimension == D1:
            extent_limits = [
                getattr(limits, 'max_texture_dimension_1d', 8192),
                1,
                1
            ]
            sample_limit = 1
