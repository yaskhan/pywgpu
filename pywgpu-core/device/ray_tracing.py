...
class RayTracing:
    ...
    def create_tlas(self, device: Any, desc: TlasDescriptor) -> Any:
        ...
        try:
            ...
            self.tlas_counter += 1
            tlas = {"id": self.tlas_counter, "label": desc.label, "size": desc.size, "type": "tlas"}
            self.tlas_registry[self.tlas_counter] = tlas
            return tlas
        
        # Get HAL device
        hal_device = getattr(device, 'hal_device', None) or getattr(device, '_hal_device', None)
    ...
    def create_blas(self, device: Any, desc: BlasDescriptor) -> Any:
        ...
        try:
            ...
            # Fallback to placeholder if HAL not available
            self.blas_counter += 1
            blas = {"id": self.blas_counter, "label": desc.label, "size": desc.size, "type": "blas"}
