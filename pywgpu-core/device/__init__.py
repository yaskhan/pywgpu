...
class DeviceLogic(Device):
    ...
    def create_render_bundle_encoder(self, desc: Any) -> Any:
        ...
        if hasattr(self.ops, 'device_create_render_bundle_encoder'):
            device_id = self.get_device_id()
            if device_id is not None:
                encoder_id, error = self.ops.device_create_render_bundle_encoder(device_id, desc)
                if error:
                    raise RuntimeError(f"Failed to create render bundle encoder: {error}")
                return encoder_id
        return None
    ...
    def finish_render_bundle(self, bundle_encoder: Any, desc: Any) -> Any:
        ...
        if hasattr(self.ops, 'render_bundle_encoder_finish'):
            bundle_id, error = self.ops.render_bundle_encoder_finish(bundle_encoder, desc)
            if error:
