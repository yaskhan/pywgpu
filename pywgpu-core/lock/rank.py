...
COMMAND_BUFFER_DATA = LockRank(
    name="CommandBuffer::data",
    bit=1 << 0,
    followers=set()  # Will be populated below
)

# Device locks
DEVICE_SNATCHABLE_LOCK = LockRank(
    name="Device::snatchable_lock",
    bit=1 << 1,
    followers=set()
)

DEVICE_USAGE_SCOPES = LockRank(
    name="Device::usage_scopes",
    bit=1 << 2,
    followers=set()
)
...
DEVICE_COMMAND_INDICES = LockRank(
    name="Device::command_indices",
    bit=1 << 6,
    followers=set()
)

DEVICE_DEFERRED_DESTROY = LockRank(
    name="Device::deferred_destroy",
    bit=1 << 7,
    followers=set()
)

DEVICE_LOST_CLOSURE = LockRank(
    name="Device::device_lost_closure",
    bit=1 << 8,
    followers=set()
)
...
BUFFER_MAP_STATE = LockRank(
    name="Buffer::map_state",
    bit=1 << 9,
    followers=set()
)

BUFFER_BIND_GROUPS = LockRank(
    name="Buffer::bind_groups",
    bit=1 << 10,
    followers=set()
)

BUFFER_INITIALIZATION_STATUS = LockRank(
    name="Buffer::initialization_status",
    bit=1 << 11,
    followers=set()
)
...
QUEUE_PENDING_WRITES = LockRank(
    name="Queue::pending_writes",
    bit=1 << 12,
    followers=set()
)

QUEUE_LIFE_TRACKER = LockRank(
    name="Queue::life_tracker",
    bit=1 << 13,
    followers=set()
)
...
COMMAND_ALLOCATOR_FREE_ENCODERS = LockRank(
    name="CommandAllocator::free_encoders",
    bit=1 << 14,
    followers=set()
)
...
SHARED_TRACKER_INDEX_ALLOCATOR_INNER = LockRank(
    name="SharedTrackerIndexAllocator::inner",
    bit=1 << 15,
    followers=set()
)
...
RESOURCE_POOL_INNER = LockRank(
    name="ResourcePool::inner",
    bit=1 << 17,
    followers=set()
)

IDENTITY_MANAGER_VALUES = LockRank(
    name="IdentityManager::values",
    bit=1 << 18,
    followers=set()
)
...
TEXTURE_BIND_GROUPS = LockRank(
    name="Texture::bind_groups",
    bit=1 << 20,
    followers=set()
)

TEXTURE_INITIALIZATION_STATUS = LockRank(
    name="Texture::initialization_status",
    bit=1 << 21,
    followers=set()
)

TEXTURE_CLEAR_MODE = LockRank(
    name="Texture::clear_mode",
    bit=1 << 22,
    followers=set()
)
...
BLAS_BUILT_INDEX = LockRank(
    name="Blas::built_index",
    bit=1 << 24,
    followers=set()
)

BLAS_COMPACTION_STATE = LockRank(
    name="Blas::compaction_size",
    bit=1 << 25,
    followers=set()
)

TLAS_BUILT_INDEX = LockRank(
    name="Tlas::built_index",
    bit=1 << 26,
    followers=set()
)

TLAS_DEPENDENCIES = LockRank(
    name="Tlas::dependencies",
    bit=1 << 27,
    followers=set()
)
