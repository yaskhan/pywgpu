...
def calculate_src_buffer_binding_size(buffer_size: int, limits: dict) -> int:
    ...
    """
    max_storage_buffer_binding_size = limits.get(
        "max_storage_buffer_binding_size", 2**30
    )
    min_storage_buffer_offset_alignment = limits.get(
...
def calculate_src_offsets(
    buffer_size: int, limits: dict, offset: int
) -> Tuple[int, int]:
    ...
    dynamic_offset_stride = (
        max(0, chunks - chunk_adjustment) * min_storage_buffer_offset_alignment
    )
    ...
    src_dynamic_offset = (
        min(src_dynamic_offset_index, max_dynamic_offset_index) * dynamic_offset_stride
    )
...
class DrawBatcher:
    ...
    def add(
        ...
        stride = get_stride_of_indirect_args(
            family.name if hasattr(family, "name") else str(family)
        )

        dst_resource_index, dst_offset = (
            indirect_draw_validation_resources.get_dst_subrange(stride, self)
        )

        buffer_size = getattr(src_buffer, "size", 0)
        limits = getattr(device, "limits", {})
        src_dynamic_offset, src_offset = calculate_src_offsets(
            buffer_size, limits, offset
        )
        ...
        indexed = (
            family.name == "DRAW_INDEXED"
            if hasattr(family, "name")
            else "indexed" in str(family).lower()
        )
        entry = MetadataEntry.new(
            indexed, src_offset, dst_offset, vertex_or_index_limit, instance_limit
        )
...
class DrawResources:
    ...
    def dispose(self) -> None:
        ...
        if (
            hasattr(self.device, "indirect_validation")
            and self.device.indirect_validation
        ):
...
class Draw:
    ...
    def __init__(self, device: Any, features: dict, backend: str):
        ...
        self.module = device.create_shader_module(
            label="Indirect Draw Validation Shader", source=VALIDATE_DRAW_WGSL
        )
        ...
        supports_indirect_first_instance = features.get(
            "indirect-first-instance", False
        )
    ...
    def _create_bind_group_layout(
        self, device, read_only, has_dynamic_offset, min_binding_size
    ):
        # COMPUTE = 0x4
    ...
    def create_src_bind_group(
        self, device: Any, limits: dict, buffer_size: int, buffer: Any
    ) -> Optional[Any]:
        """
    ...
    def inject_validation_pass(
        ...
        for batch in batcher.batches.values():
            ...
            metadata_resource_index, metadata_buffer_offset = (
                resources.get_metadata_subrange(data_len, current_metadata_entry)
            )
        ...
        metadata_indices = set(
            batch.metadata_resource_index for batch in batcher.batches.values()
        )
        barriers = []
        ...
        dst_indices = set(
            batch.dst_resource_index for batch in batcher.batches.values()
        )
        ...
        for batch in batcher.batches.values():
            ...
            metadata_start = (
                batch.metadata_buffer_offset // 16
            )  # MetadataEntry is 16 bytes
            ...
            if hasattr(compute_encoder, "set_immediates"):
                compute_encoder.set_immediates(
                    self.pipeline_layout, 0, [metadata_start, metadata_count]
                )

            # Bind group 0: Metadata
            compute_encoder.set_bind_group(
                0, resources.get_metadata_bind_group(batch.metadata_resource_index)
            )

            # Bind group 1: Source Indirect Buffer
            # Needs to handle dynamic offset
            src_bind_group = batch.src_buffer.get_indirect_validation_bind_group(
                snatch_guard
            )
            compute_encoder.set_bind_group(
                1, src_bind_group, [batch.src_dynamic_offset]
            )

            # Bind group 2: Destination Validated Buffer
            compute_encoder.set_bind_group(
                2, resources.get_dst_bind_group(batch.dst_resource_index)
            )
