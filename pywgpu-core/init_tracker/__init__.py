...
class InitTracker(Generic[T]):
    ...
    def drain(self, drain_range: Range[T]) -> Iterator[Range[T]]:
        ...
        if (num_affected == 1 and 
            first_range.start < drain_range.start and 
            first_range.end > drain_range.end):
            # Split into two ranges
            old_start = first_range.start
            self.uninitialized_ranges[first_index] = Range(drain_range.end, first_range.end)
            self.uninitialized_ranges.insert(first_index, Range(old_start, drain_range.start))
        else:
            # Adjust border ranges and delete everything in-between
            remove_start = first_index
            if first_range.start < drain_range.start:
                self.uninitialized_ranges[first_index] = Range(first_range.start, drain_range.start)
                remove_start = first_index + 1
            
            last_range = self.uninitialized_ranges[index - 1]
            remove_end = index
            if last_range.end > drain_range.end:
                self.uninitialized_ranges[index - 1] = Range(drain_range.end, last_range.end)
                remove_end = index - 1
