...
class Writer:
    ...
    def _initialize_names(self) -> None:
        ...
        reserved_keywords = [
            ...
            "RWStructuredBuffer",
            "RWByteAddressBuffer",
            "RWAppendStructuredBuffer",
            "RWConsumeStructuredBuffer",
            "SamplerState",
    ...
    def _write_struct_definition(self, handle: Any, members: List[Any]) -> None:
        ...
        for i, member in enumerate(members):
            ...
            if (
                hasattr(member, "binding")
                and member.binding
                and hasattr(member.binding, "location")
            ):
    ...
    def _write_compute_entry_point(self) -> None:
        ...
        self.out.write(
            f"void {func_name}(uint3 dispatchThreadID : SV_DispatchThreadID) {{\n"
        )
    ...
    def _write_function(self, function: Any) -> None:
        ...
        return_type = self._type_to_hlsl(
            function.result.ty if function.result else None
        )
        ...
        for stmt in body:
            ...
            if stmt_type == "LocalVariable":
                ...
                if hasattr(stmt, "pointer") and hasattr(stmt, "value"):
                    self.out.write(
                        f"{indent}{self._expression_to_hlsl(stmt.pointer)} = {self._expression_to_hlsl(stmt.value)};\n"
                    )
            elif stmt_type == "Return":
                if hasattr(stmt, "value") and stmt.value:
                    self.out.write(
                        f"{indent}return {self._expression_to_hlsl(stmt.value)};\n"
                    )
                else:
    ...
    def _type_to_hlsl(self, ty: Any) -> str:
        ...
        if str(inner).startswith("Scalar."):
            ...
            scalar = (
                self._type_to_hlsl(inner.scalar)
            ...
            element = (
                self._type_to_hlsl(inner.element)
