...
class ExternalTextureBinding:
    ...
    def __init__(
        self, y_plane: int, cbcr_plane: int, alpha_plane: Optional[int] = None
    ):
...
class Writer:
    ...
    def _initialize_names(self) -> None:
        ...
        reserved_keywords = [
            ...
            "object_space_normal",
            "object_space_tangent",
            "object_space_bitangent",
            "object_space_velocity",
            "world_space_position",
            "world_space_normal",
            "world_space_tangent",
            "world_space_bitangent",
            "world_space_velocity",
            "object_space_position_isosphere",
            "world_space_position_isosphere",
        ]
    ...
    def _write_vertex_entry_point(self) -> None:
        ...
        self.out.write(
            f"vertex VertexOutput {func_name}(VertexInput input [[stage_in]]) {{\n"
        )
    ...
    def _write_fragment_entry_point(self) -> None:
        ...
        self.out.write(
            f"fragment float4 {func_name}(FragmentInput input [[stage_in]]) {{\n"
        )
    ...
    def _write_compute_entry_point(self) -> None:
        ...
        self.out.write(
            f"    uint3 thread_position_in_grid [[thread_position_in_grid]],\n"
        )
        self.out.write(
            f"    uint3 threadgroup_position_in_grid [[threadgroup_position_in_grid]],\n"
        )
        ...
        for stmt in body:
            ...
            if stmt_type == "LocalVariable":
                ...
                if hasattr(stmt, "pointer") and hasattr(stmt, "value"):
                    self.out.write(
                        f"{indent}{self._expression_to_msl(stmt.pointer)} = {self._expression_to_msl(stmt.value)};\n"
                    )
            elif stmt_type == "Return":
                if hasattr(stmt, "value") and stmt.value:
                    self.out.write(
                        f"{indent}return {self._expression_to_msl(stmt.value)};\n"
                    )
                else:
    ...
    def _type_to_msl(self, ty: Any) -> str:
        ...
        if str(inner).startswith("Scalar."):
            ...
            scalar = (
                self._type_to_msl(inner.scalar) if hasattr(inner, "scalar") else "float"
            )
            ...
            element = (
                self._type_to_msl(inner.element)
