...
class SpvParser(Parser):
    ...
    def parse(
        self, source: Union[bytes, Any], options: Optional[Dict[str, Any]] = None
    ) -> Module:
        """
    ...
    def _parse_instructions(self, data: bytes) -> List[Any]:
        ...
        while offset < len(data):
            ...
            word_count = (
                int.from_bytes(data[offset : offset + 4], byteorder="little") >> 16
            )
            opcode = (
                int.from_bytes(data[offset : offset + 4], byteorder="little") & 0xFFFF
            )

            # Extract instruction words
            instruction_bytes = data[offset : offset + word_count * 4]
            instructions.append(
                {"opcode": opcode, "word_count": word_count, "data": instruction_bytes}
            )
