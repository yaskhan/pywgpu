...
class ConstantError(ValidationError):
    ...
    def __init__(
        self, message: str, handle: Optional[int] = None, name: Optional[str] = None
    ) -> None:
        """
...
class OverrideError(ValidationError):
    ...
    INITIALIZER_EXPR_TYPE = (
        "Initializer must be a const-expression or override-expression"
    )
...
class EntryPointError(ValidationError):
    ...
    def __init__(
        self, message: str, stage: Optional[str] = None, name: Optional[str] = None
    ) -> None:
        """
