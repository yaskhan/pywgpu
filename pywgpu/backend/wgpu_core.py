...
class WgpuCoreBackend:
    ...
    async def request_adapter(self, options: Optional[RequestAdapterOptions] = None) -> Optional['WgpuCoreAdapter']:
        """
...
class WgpuCoreAdapter:
    ...
    async def request_device(self, descriptor: Optional[DeviceDescriptor] = None) -> Tuple['WgpuCoreDevice', 'WgpuCoreQueue']:
        await asyncio.sleep(0)
        # Global.adapter_request_device returns (device_id, queue_id)
        device_id, queue_id = self._global.adapter_request_device(self._adapter_id, descriptor)
        return WgpuCoreDevice(self._global, device_id), WgpuCoreQueue(self._global, queue_id)
...
class WgpuCoreDevice:
    ...
    def create_buffer(self, descriptor: Any) -> 'WgpuCoreBuffer':
        buffer_id, error = self._global.ops.device_create_buffer(self._device_id, descriptor)
        if error:
    ...
    def create_texture(self, descriptor: Any) -> 'WgpuCoreTexture':
        texture_id, error = self._global.ops.device_create_texture(self._device_id, descriptor)
        if error:
    ...
    def create_command_encoder(self, descriptor: Any) -> 'WgpuCoreCommandEncoder':
        encoder_id, error = self._global.ops.device_create_command_encoder(self._device_id, descriptor)
        if error:
    ...
    def create_shader_module(self, descriptor: Any) -> 'WgpuCoreShaderModule':
        ...
        shader_id, error = self._global.ops.device_create_shader_module(self._device_id, descriptor, source)
        if error:
    ...
    def create_render_pipeline(self, descriptor: Any) -> 'WgpuCoreRenderPipeline':
        pipeline_id, error = self._global.ops.device_create_render_pipeline(self._device_id, descriptor)
        if error:
    ...
    def create_compute_pipeline(self, descriptor: Any) -> 'WgpuCoreComputePipeline':
        pipeline_id, error = self._global.ops.device_create_compute_pipeline(self._device_id, descriptor)
        if error:
    ...
    def create_pipeline_layout(self, descriptor: Any) -> 'WgpuCorePipelineLayout':
        layout_id, error = self._global.ops.device_create_pipeline_layout(self._device_id, descriptor)
        if error:
    ...
    def create_bind_group_layout(self, descriptor: Any) -> 'WgpuCoreBindGroupLayout':
        layout_id, error = self._global.ops.device_create_bind_group_layout(self._device_id, descriptor)
        if error:
    ...
    def create_bind_group(self, descriptor: Any) -> 'WgpuCoreBindGroup':
        group_id, error = self._global.ops.device_create_bind_group(self._device_id, descriptor)
        if error:
    ...
    def create_sampler(self, descriptor: Any) -> 'WgpuCoreSampler':
        sampler_id, error = self._global.ops.device_create_sampler(self._device_id, descriptor)
        if error:
    ...
    def create_pipeline_cache(self, descriptor: Any) -> 'WgpuCorePipelineCache':
        cache_id, error = self._global.ops.device_create_pipeline_cache(self._device_id, descriptor)
        if error:
