...
class BlendComponent(BaseModel):
    ...
    def uses_constant(self) -> bool:
        ...
        """
        return self.src_factor in {BlendFactor.CONSTANT, BlendFactor.ONE_MINUS_CONSTANT} or \
               self.dst_factor in {BlendFactor.CONSTANT, BlendFactor.ONE_MINUS_CONSTANT}
...
class BlendState(BaseModel):
    ...
    def PREMULTIPLIED_ALPHA_BLENDING(cls) -> 'BlendState':
        ...
        return cls(
            color=over_component,
            alpha=over_component
        )
...
class StencilFaceState(BaseModel):
    ...
    def needs_ref_value(self) -> bool:
        ...
        return self.compare.needs_ref_value() or \
               self.fail_op == StencilOperation.REPLACE or \
               self.depth_fail_op == StencilOperation.REPLACE or \
               self.pass_op == StencilOperation.REPLACE
    
    def is_read_only(self) -> bool:
        """Returns true if the face state doesn't mutate the target values."""
        return (self.pass_op == StencilOperation.KEEP and
                self.depth_fail_op == StencilOperation.KEEP and
...
class StencilState(BaseModel):
    ...
    def is_enabled(self) -> bool:
        ...
        return (self.front != StencilFaceState.IGNORE() or 
                self.back != StencilFaceState.IGNORE()) and \
               (self.read_mask != 0 or self.write_mask != 0)
...
class DepthBiasState:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (self.constant == other.constant and
                float_to_bits(self.slope_scale) == float_to_bits(other.slope_scale) and
                float_to_bits(self.clamp) == float_to_bits(other.clamp))
    ...
    def __hash__(self) -> int:
        ...
        return hash((
            self.constant,
            float_to_bits(self.slope_scale),
            float_to_bits(self.clamp)
        ))
...
class DrawIndirectArgs:
    ...
    def as_bytes(self) -> bytes:
        ...
        import struct
        return struct.pack('<4I', self.vertex_count, self.instance_count,
...
class DrawIndexedIndirectArgs:
    ...
    def as_bytes(self) -> bytes:
        """Return bytes representation of struct, ready to be written in a buffer."""
        import struct
        return struct.pack('<4Ii', self.index_count, self.instance_count,
                      self.first_index, self.base_vertex, self.first_instance)
